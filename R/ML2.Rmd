...
```{r include=TRUE}
# PRE MOBER 
otu_clr_mat <- clrInv(otu_clr)
class(otu_clr_mat)
otu_clr_mat <- as.matrix(otu_clr_mat)
otu_table_clr <- otu_table(otu_clr_mat, taxa_are_rows = TRUE)

physeq_pre_abr <- phyloseq(t(otu_abr), sample_data(physeq_agl_combined))
physeq_pre_clr <- phyloseq(t(otu_table_clr), sample_data(physeq_agl_combined))



physeq_pre <- subset_samples(physeq_pre_abr, sample_data(physeq_pre_abr)$status != "adenoma")
